...
class TimewalkScan(ScanBase):
    ...
    def scan(self, pix_list=[200], mask_steps=4, repeat_command=101, columns=[True] * 16, scan_range=[0, 1.2, 0.1],
             vthin1Dac=80, preCompVbnDac=50, mask_filename='', **kwargs):
        '''Scan loop
    ...
    def tdc_table(self):
        ...
        with tb.open_file(h5_filename, 'r+') as in_file_h5:
            ...
            avg_tab = np.rec.fromarrays([charge_list, pxl_list, avg_tdc, avg_tdc_err, avg_del, avg_del_err],
                                        dtype=[('charge', float), ('pixel_no', int), ('tot_ns', float),
                                               ('err_tot_ns', float), ('delay_ns', float), ('err_delay_ns', float)])
