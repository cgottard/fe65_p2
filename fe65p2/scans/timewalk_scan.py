...
class TimewalkScan(ScanBase):
    ...
    def scan(self, mask_steps=1, repeat_command=1e6, columns=[True] + [False] * 15, **kwargs):
        ...
        self.dut['INJ_HI'].set_voltage(0.0, unit='V')
        ...
        self.dut['global_conf']['vffDac'] = 42  # Preamp feedback bias (def 42)
        ...
        self.dut['global_conf']['ColEn'][:] = bitarray.bitarray(
            columns)  # Enables column (If '0' col does not receive clk, trigger, etc)
        self.dut.write_global()
        ...
        self.dut['inj'].set_width(1000)  # 100
        self.dut['inj'].set_repeat(1000)
        ...
        with self.readout():
            ...
            self.dut['global_conf']['InjEnLd'] = 1
            self.dut.write_global()
            self.dut['global_conf']['vthin1Dac'] = 80  # global threshold ctrl 1
            self.dut.write_global()
        ...
        print 'PORCAMADONNA'
        print self.dut['tdc'].get_event_counter()
    ...
    def analyze(self):
        ...
        with tb.open_file(h5_filename, 'r+') as in_file_h5:
            ...
            tdc_data = raw_data & 0xFFF  # take last 12 bit
            tdc_delay = (raw_data & 0x0FF00000) >> 20

            hist = np.bincount(tdc_data)
            print 'bincount', hist, hist.shape  # , np.sum(np.bincount(tdc_data>20))
            print np.max(tdc_data), raw_data.shape

            for i in range(100):
                if (tdc_data[i + 100] > 10):
                    print  hex(raw_data[i + 100]), tdc_data[i + 100], tdc_delay[i + 100]
